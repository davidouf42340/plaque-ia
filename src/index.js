...
// ─── Enregistrement des polices custom ───────────────────────────────────────
// Si tu as un fichier de police dans src/fonts/, il sera enregistré ici.
// Sinon @napi-rs/canvas utilisera ses polices embarquées (sans-serif correct).
const fontFiles = [
  { file: "script.ttf",     family: "ScriptCustom" },
  { file: "arialblack.ttf", family: "ArialBlack"   },
...
for (const { file, family } of fontFiles) {
  ...
  if (fs.existsSync(fontPath)) {
    ...
    console.log(`Police enregistrée : ${family} (${fontPath})`);
  }
}
// ─────────────────────────────────────────────────────────────────────────────
...
function normalizeThickness(value = "") {
  return String(value).trim().toLowerCase().replace("mm", "").replace(",", ".").trim();
}
...
async function saveCreationBatch({ prompt, category, creations = [] }) {
  ...
  const galleryIdx = pickGalleryIndex(prompt, creations);
...
function detectCategory(prompt = "") {
  ...
  for (const rule of CATEGORY_RULES) {
    if (rule.words.some(w => p.includes(w))) return rule.key;
  }
  return "divers";
}
...
async function fitLogo(buffer, boxWidth, boxHeight, colorHex = "#111111") {
  ...
  const colored = await sharp({ create: { width: logoW, height: logoH, channels: 3, background: { r,g,b } } })
...
function getFontName(fontKey = "sans") {
  const map = {
    sans:   "Arial Black",
    serif:  "Georgia",
    mono:   "Courier New",
  ...
  return map[fontKey] || "Arial Black";
}

// ─── TEXTE PRODUCTION via @napi-rs/canvas ────────────────────────────────────
// Rendu pixel-perfect, texte centré, taille automatique pour remplir la zone.
// fontSize / textScale / logoScale ignorés — production indépendante du visuel.
function buildProductionTextCanvas({
  width,          // largeur zone texte
  height,         // hauteur canvas (PRODUCTION_HEIGHT)
  line1 = "",
  line2 = "",
  line3 = "",
  fontFamilies = {}
  ...
  // Canvas transparent pour cette zone
  ...
  // ── Réduction si texte trop long ──────────────────────────────────────────
  // On mesure la vraie largeur avec canvas.measureText pour être précis
  let fontSize = Math.round(baseFontSize);
  const fontKey = fontFamilies["line1"] || "sans";
  const fontName = getFontName(fontKey);

  // Recherche de la taille optimale par dichotomie
  ...
  fontSize = Math.max(lo, Math.round(height * 0.08));
  ...
  lines.forEach((line, i) => {
    const lFontName = getFontName(fontFamilies[line.key] || "sans");
    ctx.font = `700 ${fontSize}px "${lFontName}"`;
...
async function buildProductionComposite({
  ...
  // Logos en noir #111111 (gravure laser)
  if (leftLogoUrl) {
  ...
  // Texte via canvas — rendu pixel-perfect
  const textBuffer = buildProductionTextCanvas({
    width:  textWidth,
    height,
    line1,
    line2,
...
async function getShopifyAdminAccessToken() {
  ...
  const response = await fetch(`https://${shop}/admin/oauth/access_token`, { method:"POST", headers:{"Content-Type":"application/x-www-form-urlencoded"}, body:body.toString() });
...
async function getShopifyFileById(fileId) {
  const data = await shopifyGraphQL(`
    query getFile($id: ID!) { node(id: $id) { __typename
...
app.post("/api/logos/search-or-generate", async (req, res) => {
  try {
    ...
    for (let i = 0; i < (result.data||[]).length; i++) {
      ...
      try { const u = await uploadImageToShopify(buffer, fileName, `Logo IA: ${cleanPrompt}`); shopifyUrl = u.url; shopifyFileId = u.id; }
...
// - Canvas fixe 1181x295 (100x25mm) — dimension commandée ignorée
// - Texte rendu via @napi-rs/canvas (pixel-perfect, polices natives)
// - fontSize / textScale / logoScale ignorés
// - Upload Shopify → { url, shopifyUrl, localUrl }
...
app.get("/api/gallery/categories", async (req, res) => {
  try { const items = await getAllGalleryItemsForCategories(); res.json({ categories: getGalleryCategories(items) }); }
...
app.get("/api/gallery", async (req, res) => {
  try {
    ...
    const items        = itemsRaw.map(item => ({ id:item.id, preview:item.image_url, prompt:item.prompt, category:item.category||"divers", imageUrl:item.image_url, shopifyUrl:item.shopify_url||null, localUrl:item.local_url||null, createdAt:item.created_at }));
...
app.get("/api/gallery/random", async (req, res) => {
  try {
    ...
    const allForCats   = await getAllGalleryItemsForCategories();
    const items        = randomItems.map(item => ({ id:item.id, preview:item.image_url, prompt:item.prompt, category:item.category||"divers", imageUrl:item.image_url, shopifyUrl:item.shopify_url||null, localUrl:item.local_url||null, createdAt:item.created_at }));
